...
export const getAccount = createAsyncThunk("account/get", async (token) => {
  try {
    const res = await api
      .get("api/account", {
        headers: {
...
export const logout = createAsyncThunk("account/logout", async (token) => {
  try {
...
export const updateUser = createAsyncThunk(
  ...
  async ({ token, id, values }) => {
    try {
      const res = await api
        .put(`api/account/${id}/update`, values, {
          headers: {
            Authorization: token,
          },
        })
