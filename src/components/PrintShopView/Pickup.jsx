...
import { deliverOrder, getApprovedOrders, getDeliveredOrders, getProcessingOrder, getReadyForPickUpOrders } from "../../../store/PrintShop/Thunks";
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faCheckCircle, faEnvelopeOpen, faHandsHoldingCircle, faStickyNote, faTruckPickup } from '@fortawesome/free-solid-svg-icons';
...
const Pickup = ({ readyForPickupOrders }) => {
    ...
    const markOrderAsDelivered = async (id) => {
        ...
        if (pickUpName) {
            ...
            const updatedOrder = api.put(`/api/orders/${id}/picked-up-by`, { name: pickUpName }, {
                headers: {
                    Authorization: token
                }
            }).then(async (res) => {
        ...
        <div>
            {
                readyForPickupOrders ?
                    readyForPickupOrders.map((r, index) => (
                        <div key={r._id}>
                            <Divider orientation="left" className="flex"> Department: {user.length > 0 ? <span>{getUser(r.creatorId)}</span> : <RingLoader size={6} />} </Divider>
                            <Collapse size="large">
                                <Panel header={`${r.creatorName} - ${r._id} - ${formatDate(r.createdOn)}`} key={r._id} extra={
                                    <div>
                                        {
                                            r.notes || r.notes != '' ?
                                        ...
                                        {
                                            <Tooltip placement="top" title={`Update ${r.creatorName}'s Order as Delivered?`}>
                                                <button
                                                    onClick={(event) => {
                                            ...
                                            pdf[index].map((p, i) => (
                                                <div key={i} className="mb-5 border-b">
                                                    ...
                                                    <div className="flex justify-center">
                                                        <iframe src={`/api/getOrders?filePath=${sanitizePath(r.finalOrderPaths[i])}`} className="w-11/12"></iframe>
                                                    </div>
                                                    <div className="text-center mb-3 mt-3">QTY to be Printed: {(r?.labels[i]?.qty * p.unitPack)}</div>
