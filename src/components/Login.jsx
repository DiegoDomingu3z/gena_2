                    ...
                    {({ isSubmitting }) => (
                        <Form className="space-y-4 md:space-y-6" >
                            ...
                            <div>
                                ...
                                <div className='relative'><Field type={showPassword ? 'text' : 'password'} name="password" id="password" placeholder="••••••••" className={`bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 ${user.account === 'INCORRECT Password' && 'animate-wiggle border-red-500'}`} required="" />{!showPassword && <FontAwesomeIcon className='absolute top-[35%] right-[6%] hover:cursor-pointer' onClick={() => setShowPassword(true)} icon={faEye} />}{showPassword && <FontAwesomeIcon className='absolute top-[35%] right-[5.9%] hover:cursor-pointer' onClick={() => setShowPassword(false)} icon={faEyeSlash} />}</div>
                            </div>
