  ...
  return (
    <>
      <div className='w-full flex flex-col justify-center mb-auto'>
        <Link href={'/start-new-order'} className={router.pathname === '/' && user.user ? "flex items-center gap-5 text-sm" : "hidden"}>
        ...
        <Link href={'/current-orders'} className={router.pathname === '/' && user.user ? "flex items-center gap-5 text-sm" : "hidden"}>
          <button className={'h-14 w-full flex items-center gap-5 p-8 transition-all ease-in-out duration-150 text-white hover:shadow-md hover:tracking-wide hover:border-t-0 hover:border-l-0 hover:border-b-0 hover:bg-opacity-30 hover:bg-slate-900 hover:border-r-white hover:border-8'}>
      ...
      <div className='w-full h-16'>
        ...
        {<div className='text-white bg-[#1e2a38] w-full h-full flex justify-center items-center'>{user.user ? `${user.user.firstName} ${user.user.lastName}` : "Welcome to GENA!"}</div>}
      </div>
    </>
  )
}
