...
import { faPencil, faTrash, faNoteSticky, faEllipsisVertical, faXmark } from '@fortawesome/free-solid-svg-icons'
...
const OrderCard = ({ deleted, setDeleted, modalState, setModalState, blobs, setBlobs, toggleSort, openAction, setOpenAction }) => {
      ...
      {order ?
        (toggleSort === 'newest' ? [...order].sort((a, b) => new Date(b.createdOn) - new Date(a.createdOn)) : order).map((o) => {
          ...
          <div className='grid grid-cols-5 justify-items-center bg-white border-t py-5 justify-between hover:bg-slate-100 pl-2' key={o._id}>
            <p className='text-sm w-full break-words lg:text-center'>{o._id}</p>
            <p className=''>{o.labels.length}</p>
            <p className=''>{formatDate(o.createdOn)}</p>
            <span className={`px-5 ${statusColors[o.status]} text-center text-white rounded-lg flex items-center text-sm`}>{o.status}</span>
            <div className='flex gap-5 w-32 relative'>
              <Tooltip placement='top' title="Edit Order">
                <button
                  ...
                  data-order-id={o._id} className={`text-[#233043] hover:bg-[#ff9800] hidden lg:block hover:text-white transition-all ease-in-out w-7 h-7 rounded-full`}>
                  <FontAwesomeIcon icon={faPencil} /></button>
              </Tooltip>
              <Tooltip placement='top' title="Delete Order">
                <button
                  onClick={() => deleteOrder(o._id)} className={`text-[#233043] hover:bg-[#ff1b1b] hidden lg:block hover:text-white transition-all ease-in-out w-7 h-7 rounded-full ${o.status === 'processing' && 'pointer-events-none'}`}><FontAwesomeIcon icon={faTrash} /></button>
              </Tooltip>
              <button onClick={() => handleActionPopup(o._id)} className='border border-black py-1 px-4 rounded-full absolute right-12 lg:hidden'>
                <FontAwesomeIcon icon={faEllipsisVertical} />
                {
                  openAction && 
                  <div className={`absolute ${o.notes ? "-top-32" : "-top-24"} -left-12 px-2 bg-sky-950 shadow-lg rounded-md w-36`}>
                      {openAction[o._id] && 
                      <div className='py-2'>
                        <button
                        onClick={() => {
                        ...
                        <button
                        onClick={() => deleteOrder(o._id)} className={`text-white hover:bg-[#ff1b1b] hover:text-white transition-all w-full ease-in-out rounded-full ${o.status === 'processing' && 'pointer-events-none'}`}>
                          <div className='flex gap-5 items-center text-sm'>
                        ...
                        {o.notes ?
                          <div>
                            <div className='border-t-[1px] w-full mb-2 mt-2'></div>
                            <button onClick={() => userNotes(o.notes, o._id)} className='text-white hover:bg-[#233043] hover:text-white transition-all ease-in-out w-full rounded-full'>
                              <div className='flex gap-5 items-center text-sm'>
